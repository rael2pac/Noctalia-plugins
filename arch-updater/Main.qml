...
Item {
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                if (pluginApi.pluginSettings.cleanupAfterUpdate ?? pluginApi.manifest.metadata.defaultSettings.cleanupAfterUpdate) {
                    Logger.i("Arch Updater", "Running cleanup...")
                    if (pluginApi.pluginSettings.toast ?? pluginApi.manifest.metadata.defaultSettings.toast ?? true) {
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                Logger.i("Arch Updater", "Cleanup completed")
                if (pluginApi.pluginSettings.toast ?? pluginApi.manifest.metadata.defaultSettings.toast ?? true) {
                    ToastService.showNotice(pluginApi.tr("toast.cleanupDone"))
                }
                refresh()
            }
        }
